#!/usr/bin/env python3
"""
Task 14: Change Password of User fedor (VIA NETCONF/YANG)
...
✓ External XML payload file management
...
Description: Change the password of existing user "fedor" via NETCONF/YANG.
XML payload is loaded from external file: task_14_payload.xml
"""
...
# ============================================================
# Load XML Payload from External File
# ============================================================
# Get script directory and build path relative to script location
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PAYLOAD_FILE = os.path.join(SCRIPT_DIR, "task_14_payload.xml")

if not os.path.exists(PAYLOAD_FILE):
    logger.error(f"✗ Payload file '{PAYLOAD_FILE}' not found!")
    logger.error(f"   Please create 'task_14_payload.xml' in the same directory as this script.")
...
try:
    with open(PAYLOAD_FILE, 'r') as f:
        XML_PAYLOAD = f.read()
    logger.info(f"✓ Loaded XML payload from: {PAYLOAD_FILE}")
except Exception as e:
    logger.error(f"✗ Failed to read payload file: {e}")
    sys.exit(1)

# ============================================================
# Verification Filter - Get User Configuration
# ============================================================
VERIFY_FILTER = """
<filter>
...
def apply_configuration(mgr):
    ...
    Uses edit-config to merge user password configuration to running datastore.
    ...
    try:
        ...
        logger.info("Configuration: User 'fedor' password change")
...
def verify_configuration(mgr):
    ...
    Uses GET with filter to retrieve and display current user configuration.
    ...
    try:
        logger.info("=" * 70)
        logger.info("STEP 2: VERIFICATION - GET RUNNING-CONFIG (User fedor)")
        logger.info("=" * 70)
...
def main():
    ...
    print("=" * 70)
    print("TASK 14: CHANGE PASSWORD OF USER FEDOR (VIA NETCONF/YANG)")
    print("=" * 70)
    ...
    try:
        ...
        if success:
            ...
            print("✓ XML Payload: Loaded from external file")
