...
Description: Voer een YANG action uit, bijvoorbeeld het wissen van interface counters.
...
def main():
    ...
    print("=" * 60)
    print(f"Task 32: Execute YANG Action")
    print("=" * 60)
    ...
    try:
        ...
        if apply_configuration(mgr):
            logger.info("Task 32 completed successfully!")
            
            # Verify configuration
            verify_configuration(mgr)
        else:
            logger.error("Task 32 failed!")
            sys.exit(1)
