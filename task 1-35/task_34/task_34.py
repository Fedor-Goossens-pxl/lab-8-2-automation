...
Description: Configureer interface-instellingen met OpenConfig YANG-modellen in plaats van IOS-XE native modellen.
...
def main():
    ...
    print("=" * 60)
    print(f"Task 34: Use OpenConfig Models")
    print("=" * 60)
    ...
    try:
        ...
        if apply_configuration(mgr):
            logger.info("Task 34 completed successfully!")
            
            # Verify configuration
            verify_configuration(mgr)
        else:
            logger.error("Task 34 failed!")
            sys.exit(1)
