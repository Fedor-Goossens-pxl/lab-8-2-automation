...
class DeviceConfig:
    ...
    TIMEOUT = 180  # 180 seconden voor OSPF/Loopback config

class ConfigFile:
    """Configuration file management - from GitHub"""
    # GitHub raw content URL (simulates real GitHub repo)
    GITHUB_REPO = "https://raw.githubusercontent.com/Fedor-Goossens-pxl/lab-8-2-automation/main/task36-netconf"
    FILENAME = "task36_config_complete.xml"
    
    @staticmethod
    def fetch_from_github():
        """Fetch XML config from GitHub repository"""
        github_url = f"{ConfigFile.GITHUB_REPO}/{ConfigFile.FILENAME}"
        
        try:
            logger.info(f"📥 Fetching configuration from GitHub...")
            logger.info(f"   URL: {github_url}")
            
            response = requests.get(github_url, timeout=10)
            response.raise_for_status()
            
            logger.info(f"✓ Configuration downloaded from GitHub ({len(response.text)} bytes)")
            ...
            return ConfigFile.find_local()
    
    @staticmethod
    def find_local():
        """Find config file locally as fallback"""
        search_paths = [
        ...
        for path in search_paths:
            if path.exists():
                logger.info(f"✓ Local config file found: {path}")
                with open(path, 'r') as f:
                    return f.read()
        
        logger.error(f"✗ Config file not found locally either!")
        return None
...
def load_configuration():
    """Load YANG XML configuration from GitHub repository"""
    print_section("1️⃣", "Load Configuration from 'GitHub'")
    
    # Fetch from GitHub (with local fallback)
    config_xml = ConfigFile.fetch_from_github()
    
    if not config_xml:
        logger.error(f"✗ Failed to load configuration from GitHub or local fallback")
        sys.exit(1)
    
    file_size = len(config_xml)
    logger.info(f"✓ Configuration loaded successfully ({file_size} bytes)")
    logger.info(f"  Source: GitHub repository")
